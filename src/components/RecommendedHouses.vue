<template>
    <div v-if="recommendedHouses.length">
        ...
        <div>
            <house-card v-for="house in recommendedHouses" :key="house.id" :house="house" customClass="recommended-houses-card" />
        </div>
    </div>
</template>
        ...
        async fetchRecommendedHouses() {
            try {
                ...
                if (response && response.data) {
                    this.recommendedHouses = response.data.slice(0, 3); // Take the first 3 houses
                }
            } catch (error) {
...
.recommended-houses-card .listing-info,.recommended-house-location,.houses-address{
    font-size: 14px;
}
...
@media screen and (max-width: 1200px) {
    .recommended-houses-card .houses-details,.house-card-details {
